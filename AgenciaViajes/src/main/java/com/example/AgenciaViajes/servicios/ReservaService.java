...
public class ReservaService {
    ...
    @Transactional
    public Reserva crearDesdeCarrito(Carrito carrito, String username, LocalDate fechaViaje) {
        if (carrito.isVacio()) {
            throw new IllegalStateException("El carrito está vacío.");
        }
        validarFechaViaje(fechaViaje);
        ...
        for (ItemCarrito item : carrito.getItems()) {
            // Se relee el paquete de la BD: el precio del carrito podría estar desactualizado
            Paquete paquete = paqueteRepo.findById(item.getIdPaquete())
                    .orElseThrow(() -> new IllegalStateException(
                            "El paquete '" + item.getNombre() + "' ya no existe."));
    ...
    public List<Reserva> buscar(Integer idCliente, Integer idDestino, EstadoReserva estado,
    ...
    public List<Reserva> misReservas(String username) {
        Cliente cliente = clienteRepo.findByUsuarioNombreUsuario(username)
                .orElseThrow(() -> new IllegalStateException("Tu usuario no tiene un cliente asociado."));
        return reservaRepo.findByClienteIdOrderByFechaReservaDesc(cliente.getIdCliente());
    }
